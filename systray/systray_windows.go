...
var (
	kernel32         = windows.MustLoadDLL("kernel32")
...
func New() (*Systray, error) {
	...
	mhwnd, _, _ := CreateWindowEx.Call(
		WS_EX_CONTROLPARENT,
		uintptr(unsafe.Pointer(windows.StringToUTF16Ptr(MainClassName))),
		0,
	...
	hwnd, _, _ := CreateWindowEx.Call(
		0,
		uintptr(unsafe.Pointer(windows.StringToUTF16Ptr(NotifyIconClassName))),
		0,
		0,
		0,
		0,
...
func NewIconFromFile(filePath string) (uintptr, error) {
	...
	hicon, _, _ := LoadImage.Call(
		0,
		uintptr(unsafe.Pointer(windows.StringToUTF16Ptr(absFilePath))),
		IMAGE_ICON,
		0,
		0,
