...
// https://github.com/infobloxopen/go-trees/tree/master/iptree
package iptree
...
func (t *Tree) GetByNet(n *net.IPNet) (interface{}, bool) {
	...
	if MSKey, MSBits, LSKey, LSBits := iPv6NetToUint64Pair(n); MSBits >= 0 {
		...
		v, ok = (*Node64)(s).Match(LSKey, LSBits)
		if ok {
			return v, ok
		}

		return t.root64.Match(MSKey, Key64BitSize-1)
	}
...
func (t *Tree) enumerate(ch chan Pair) {
	...
	for n := range t.root64.Enumerate() {
		...
		if s, ok := n.Value.(subTree64); ok {
			for n := range (*Node64)(s).Enumerate() {
				LSIP := unpackUint64ToIP(n.Key)
				mask := net.CIDRMask(Key64BitSize+int(n.Bits), iPv6Bits)
				ch <- Pair{
					Key: &net.IPNet{
