...
func Handshake(conn net.Conn, tgt net.Addr, cmd byte, auth *Auth) (socks.Addr, error) {
	...
	if addr, ok := tgt.(socks.Addr); ok {
		...
		addr, err := socks.ResolveAddrBuffer(tgt, b[3:])
		if err != nil {
	...
	return socks.ReadAddrBuffer(conn, b)
}
